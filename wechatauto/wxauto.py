...
class WeChat(WeChatBase):
    ...
    def __init__(
        ...
        print(f'usedmsgid：{self.usedmsgid}')
    ...
    def GetNextNewMessage(self, savepic=False, savefile=False, savevoice=False, timeout=10,checkRedMessage=True):
        """获取下一个新消息"""
        def check_current_chat_new_message():
            msgs_ = self.GetAllMessage()
        ...
        # 首先尝试获取当前聊天窗口的新消息
        current_chat_msgs = check_current_chat_new_message()
        if current_chat_msgs is not None:
            return current_chat_msgs

        if checkRedMessage and self.CheckNewMessage():
            ...
            while True:
                if time.time() - t0 > timeout:
                    wxlog.debug('获取新消息超时')
                    return {}
                random_move_to(self.A_ChatIcon)
                self.A_ChatIcon.DoubleClick(simulateMove=False)
                sessiondict = self.GetSessionList(newmessage=True)
                if sessiondict:
                    break
            for session in sessiondict:
                # 在跳转之前再次检查本聊天框又新消息产生吗
                final_current_chat_msgs = check_current_chat_new_message()
                if final_current_chat_msgs is not None:
                    return final_current_chat_msgs
                # 跳转
    ...
    def ChatWith(self, who, timeout=2):
        ...
        if who in list(sessiondict.keys())[:-1]:
            who_control = self.SessionBox.ListItemControl(RegexName=who)
            random_move_to(who_control)
            who_control.Click(simulateMove=False) #wjy 增加随机轨迹
            # self.SessionBox.ListItemControl(RegexName=who).Click(simulateMove=False) #wjy
            return who
        else:
            ...
            if target_control.Exists(timeout):
                wxlog.debug('选择完全匹配项')
                random_move_to(target_control)#wjy
