...
def random_move_to(control):
    """从当前鼠标位置随机移动到指定控件的位置"""
    current_mouse_pos = uia.GetCursorPos()
    target_rect = control.BoundingRectangle
    target_pos = get_center(target_rect)
    steps = random.randint(5, 10)  # 随机步数

    for i in range(steps):
        ratio = (i + 1) / steps
        x = int(current_mouse_pos[0] + (target_pos[0] - current_mouse_pos[0]) * ratio)
        y = int(current_mouse_pos[1] + (target_pos[1] - current_mouse_pos[1]) * ratio)
        ...
        time.sleep(random.uniform(0.05, 0.2))  # 每一步之间的随机等待时间

    # 最终精确到达目标位置
    uia.SetCursorPos(target_pos[0], target_pos[1])
...
wxlog = logging.getLogger('qianniu')
